...
- restrict/urgent 定稿即开启复测窗口：高风险 3 天、其余 7 天；到期未登记复测结论时，缺陷与优先级记录显示“逾期待复测”，但保留原决定。
- 复测结论只能由非拟制人的 reviewer/admin 登记，登记后清除逾期并追加不可变版本；同一缺陷逾期未复测时不得新建定稿决定；历史已定稿记录按定稿时间补算截止，已有版本链不改。
